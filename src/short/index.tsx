...
const ShortBreak = ({ selectedPage, setSelectedPage }: Props) => {
  const isAboveMediumScreens = useMediaQuery("(min-width: 1060px");
  const [time, setTime] = useState(5 * 60); // Initial time is 25 minutes in seconds
  ...
  useEffect(() => {
    ...
    if (isActive && time > 0) {
      ...
      setIsActive(false);
      setTime(5 * 60); // Reset to 25 minutes
    }
